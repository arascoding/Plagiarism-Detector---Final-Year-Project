...
other
  ...
  void selectNewColors(Graphics g) {
    ...
    Vector 
other
copyOfAviableColors = new Vector(aviableColors.length);
...
for (int i = 0; i < 4; i++) {
      ...
      // Und gleich Zeichnen
      g.setColor((Color)copyOfAviableColors.
other
elementAt(r));
  ...
  void doBox(Graphics g) {
    g.setColor((Color) validColors.elementAt(rightDirection));
...
undoBox(g);
  ...
  int showJump(Graphics g, int frame) {
    if (frame == 0) {
      ...
      yNew = 
other
getRandomInt(boxXY + 1, maxY - boxXY - 1);
      g.setColor(Color.black);
  ...
  public static void main(String args[]) {
    ...
    Frame 
other
mainFrame = new Frame("JumpBox - Xxxxx Xxxxxxxx, #861196");
